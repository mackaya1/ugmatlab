function ProjectileGUISolution
    ...
    % Gravity
    text(0.01,0.6,'Gravity','fontsize',10,'units','normalized');
    S.Gravity = uicontrol('style','edit','unit','normalized','position',...
        [0.18,0.575,0.1,0.05],'fontsize',10,'string','9.8');
...
function [] = Run(varargin)
    ...
    h = str2double(get(S.Height,'string')); % Convert string to double
    v = str2double(get(S.Velocity,'string')); % Get velocity
    g = str2double(get(S.Gravity,'string')); % Get gravity
    
    % Find x and y data for projectile motion
    [x,y] = ProjectileMotion(h,v,alpha,g);
