...
m=15; n=15; % includes boundary nodes, mesh spacing 1/(m-1) and 1/(n-1)
[x,y]=ndgrid((0:m-1)/(m-1),(0:n-1)/(n-1)); % matlab forms x and y lists

% Node Labelling
p=[x(:),y(:)]; % N by 2 matrix listing x,y coordinates of all N=m*n nodes
...
% Create mxn matrix for U for plotting and error estimate using reshape 
U_m=reshape(U, [m,n]) % Converts N-vector U back into mxn matrix 
...
% Plot Surface
surf(x,y,U_m) % Solution Plot



% Error
norm(U_m-u_exact,1) % gives L1 norm  
norm(U_m-u_exact,2) % gives L2 norm 
norm(U_m-u_exact,'inf') % gives infinity norm 
